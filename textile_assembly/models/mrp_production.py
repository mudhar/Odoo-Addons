...
class MrpProduction(models.Model):
    ...
    def _check_work_order_done(self):
        for production in self:
            if production.workorder_ids:
                po_reserved = False
                if all([x.has_po for x in production.mapped('workorder_ids.product_service_ids')]):
                    po_reserved = True
                production.check_work_order_done = po_reserved
    ...
    def check_wo_po(self):
        if not self.check_work_order_done:
            ...
            if product_ids:
                message = ''.join('%s\t\t\n' % line.display_name for line in product_ids)
